...
function updateFileLabel(input) {
  const zone  = input.closest('.url-field').querySelector('.file-upload-zone');
...
function handleFileDrop(event) {
  ...
  // Переносим файл в нативный input через DataTransfer
...
function buildDomainCard(r, rowIndex) {
  ...
  const domainColor = riskStr.includes('КРИТИЧЕСКИЙ') ? 'var(--c-red)'
    : riskStr.includes('ВЫСОКИЙ') ? '#ff6b35'
...
<div class="domain-card">
  ...
  <div class="domain-data" onclick="toggleExpand(this.nextElementSibling.nextElementSibling, this.nextElementSibling)">
    <div>
      ...
      ${r.check_time_sec ? `<div class="d-sub" style="margin-top:4px;color:var(--c-muted);font-size:.72rem">⏱ ${r.check_time_sec} сек.</div>` : ''}
    </div>
...
function buildDetailRows(r, rowIndex) {
  ...
  // Аналитика и Cookie
  // Российская аналитика ≠ трансграничная передача (синхронно с config.RUSSIAN_ANALYTICS)
  const RUSSIAN_ANALYTICS = new Set(['Яндекс.Метрика', 'LiveInternet', 'Roistat']);
  ...
  if (hasForeign)
    rows.push({ icon: '✕', cat: 'red', text: `Иностранные ресурсы / аналитика без уведомления РКН (трансграничная передача): <strong>${esc(r.foreign_resources.join(', '))}</strong>`, fine: FINES.foreign });
  else if (hasSystems)
    rows.push({ icon: '✓', cat: 'green', text: 'Иностранных ресурсов и иностранной аналитики не обнаружено (российская аналитика не считается трансграничной передачей)' });
  ...
  if (r.playwright_used && Array.isArray(r.trackers_before_consent) && r.trackers_before_consent.length) {
    const onlyRussian = r.trackers_before_consent.every(t => RUSSIAN_ANALYTICS.has(t));
    rows.push({
      icon: '✕', cat: 'red',
      text: `<strong>Трекеры загружаются ДО получения согласия</strong> (проверено Playwright): ${esc(r.trackers_before_consent.join(', '))}`,
      fine: onlyRussian ? FINES.cookie : FINES.foreign,
    });
...
<div id="${ownerBlockId}" style="display:none;border-top:1px solid var(--c-border);padding:var(--sp-lg) var(--sp-xl);background:var(--c-cyan-soft)">
  ...
  <div class="rec-actions" style="margin-top:var(--sp-md);display:none;gap:10px;flex-wrap:wrap;align-items:center">
    ...
    <button type="button" class="open-link-btn rec-collapse-btn" onclick="toggleRecText(event)" title="Свернуть текст рекомендаций">
...
<div id="${devBlockId}" style="display:none;border-top:1px solid var(--c-border);padding:var(--sp-lg) var(--sp-xl);background:rgba(0,194,222,.05)">
  ...
  <div class="rec-actions" style="margin-top:var(--sp-md);display:none;gap:10px;flex-wrap:wrap;align-items:center">
    ...
    <button type="button" class="open-link-btn rec-collapse-btn" onclick="toggleRecText(event)" title="Свернуть текст ТЗ">
      <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="18 15 12 9 6 15"/></svg>
      Свернуть
    </button>
  </div>
...
function showResults(results) {
  ...
  document.getElementById('stat-critical').textContent = results.filter(r => (r.risk||'').includes('КРИТИЧЕСКИЙ')).length;
...
function resetForm() {
  ...
  if (fileTitle) fileTitle.textContent = 'Выберите или перетащите файл';
...
async function getRecommendations(event, domain, blockId, docType) {
  ...
  const collapseBtn = block.querySelector('.rec-collapse-btn');
  if (collapseBtn) {
    collapseBtn.innerHTML =
      '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="18 15 12 9 6 15"/></svg> Свернуть';
  }
  btn.disabled = true;
...
function toggleRecText(event) {
  event.stopPropagation();
  const btn = event.target.closest('button') || event.target;
  const block = btn.closest('[id^="rec-owner-"], [id^="rec-dev-"]')
    || (btn.closest('.rec-actions') && btn.closest('.rec-actions').parentElement);
  if (!block) return;
  const textEl = block.querySelector('.rec-text');
  if (!textEl) return;
  ...
  btn.innerHTML = collapsed
    ? '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="18 15 12 9 6 15"/></svg> Свернуть'
    : '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="6 9 12 15 18 9"/></svg> Развернуть';
  btn.title = collapsed ? 'Свернуть текст' : 'Развернуть текст';
}
...
function formatMarkdown(text) {
  ...
  html = html.replace(/^# (.+)$/gm,
    '<div style="font-family:var(--f-display);font-size:1.15rem;font-weight:600;color:var(--c-text);margin:16px 0 6px;border-bottom:1px solid var(--c-border);padding-bottom:6px">$1</div>');
  html = html.replace(/^## (.+)$/gm,
    '<div style="font-size:.95rem;font-weight:700;color:var(--c-cyan);margin:14px 0 5px;text-transform:uppercase;letter-spacing:.05em">$1</div>');
  html = html.replace(/^### (.+)$/gm,
    '<div style="font-size:.88rem;font-weight:700;color:var(--c-text);margin:10px 0 4px">$1</div>');

  // Жирный и курсив
  html = html.replace(/\*\*([^*\n]+)\*\*/g, '<strong>$1</strong>');
  html = html.replace(/\*([^*\n]+)\*/g, '<em style="color:var(--c-muted)">$1</em>');

  // Код
  html = html.replace(/`([^`]+)`/g,
    '<code style="background:rgba(255,255,255,.07);padding:1px 6px;border-radius:3px;font-family:monospace;font-size:.85em;color:var(--c-cyan)">$1</code>');
  ...
  html = html.replace(/((?:^\|.+\|\n?)+)/gm, (tableBlock) => {
    ...
    const isHeader = (r) => r.replace(/\|/g, '').trim().match(/^[-: ]+$/);
    let out = '<table style="width:100%;border-collapse:collapse;font-size:.8rem;margin:8px 0">';
    let headDone = false;
    rows.forEach((row) => {
      if (isHeader(row)) { headDone = true; return; }
      const cells = row.split('|').slice(1, -1);
      const tag = !headDone ? 'th' : 'td';
      const style = tag === 'th'
        ? 'padding:5px 8px;border:1px solid var(--c-border);background:rgba(255,255,255,.05);font-weight:600;color:var(--c-cyan);font-size:.75rem;text-transform:uppercase;letter-spacing:.05em'
        : 'padding:5px 8px;border:1px solid var(--c-border);color:var(--c-text)';
      out += '<tr>' + cells.map(c =>
        `<${tag} style="${style}">${c.trim()}</${tag}>`
      ).join('') + '</tr>';
      if (!headDone) headDone = true;
    });
    out += '</table>';
    return out;
  });
  ...
  html = html.replace(/((?:^\d+\. .+\n?)+)/gm, (listBlock) => {
    ...
    return '<ol style="margin:6px 0 6px 20px;padding:0">' +
      items.map(i => `<li style="margin:3px 0;color:var(--c-text)">${i.replace(/^\d+\.\s+/, '')}</li>`).join('') +
      '</ol>';
  });
  ...
  html = html.replace(/((?:^[-*] .+\n?)+)/gm, (listBlock) => {
    const items = listBlock.trim().split('\n').filter(Boolean);
    return '<ul style="margin:6px 0 6px 16px;padding:0;list-style:none">' +
      items.map(i => {
        const txt = i.replace(/^[-*]\s+/, '');
        // Подсветка ❌ / ✅ / ⚠
        const color = txt.startsWith('❌') ? 'var(--c-red)'
          : txt.startsWith('✅') ? 'var(--c-green)'
          : txt.startsWith('⚠') ? 'var(--c-orange)'
          : 'var(--c-text)';
        return `<li style="margin:3px 0;color:${color};display:flex;gap:6px"><span style="color:var(--c-border);flex-shrink:0">›</span><span>${txt}</span></li>`;
      }).join('') +
      '</ul>';
  });

  // Переносы строк (оставшиеся \n)
  html = html.replace(/\n/g, '<br>');
